...
def main():
    ...
    dataset_name = dataset_config.get("name", "Trelis/big_patent_sample")
    
    # Load the TinyLlama 1.1B model
    model_name = config["models"]["base"]["name"]
    ...
    if os.path.exists(cache_file):
        ...
        if text_column not in dataset.column_names:
            ...
            raise ValueError(f"Text column '{text_column}' not found in dataset. Available columns: {available_columns}")

        # Tokenize dataset
        def tokenize_function(examples):
            outputs = tokenizer(examples[text_column], truncation=True, max_length=2048, padding="max_length")
    ...
    output_dir = config["models"]["finetuned"]["path"]
